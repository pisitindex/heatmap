#include "DialogCTMVdoHeatmap.h"
#include "ui_DialogCTMVdoHeatmap.h"
...
DialogCTMVdoHeatmap::DialogCTMVdoHeatmap(QWidget *parent) :
  QDialog(parent),
  ui(new Ui::DialogCTMVdoHeatmap)
{
  ui->setupUi(this);
...
DialogCTMVdoHeatmap::~DialogCTMVdoHeatmap()
{
  delete ui;
}

QJsonObject DialogCTMVdoHeatmap::saveState() {
  ...
  //#############################
  jso["vdo_path"] = ui->label_vdo_path->text();
  //jso["step_sec"] = ui->doubleSpinBox_step_sec->value();
  ...
  jso["speed"] = ui->comboBox_speed->currentText().toInt();
  qDebug() << ui->comboBox_speed->currentText().toInt();
  jso["speed_index"] = ui->comboBox_speed->currentIndex();
  qDebug() << jso["speed_index"];
  if(jso["speed"] == 1) jso["step_sec"] = 0.044;
  if(jso["speed"] == 2) jso["step_sec"] = 0.21;
  if(jso["speed"] == 4) jso["step_sec"] = 0.42;
  if(jso["speed"] == 10) jso["step_sec"] = 0.971;
  if(jso["speed"] == 50) jso["step_sec"] = 2.576;
  if(jso["speed"] == 200) jso["step_sec"] = 2.89;

//  int index_combobox = ui->comboBox_speed->findData(jso["speed"].toString());

  return jso;
}

void DialogCTMVdoHeatmap::restoreState(QJsonObject jso) {
  is_restoring = true;
  ...
  qDebug()<< jso["spped_index"].toInt();
  ui->comboBox_speed->setCurrentIndex(jso["speed_index"].toInt());
  ...
  //#############################
  QString vdo_path = jso["vdo_path"].toString();
  if(QFile(vdo_path).exists()) {
    tmp_video = vdo_path;
    ui->label_vdo_path->setText(vdo_path);
  }
  //ui->doubleSpinBox_step_sec->setValue(jso["step_sec"].toDouble());
...
void DialogCTMVdoHeatmap::on_pushButton_reset_count_clicked()
{
...
void DialogCTMVdoHeatmap::on_checkBox_enable_tracking_clicked(bool checked)
{
  if(checked) {
      QString name = ui->label_config_name->text();
      if(!name.isEmpty() && cira_lib_darknet_widget::dlg_detect_lst_config->map_det_cfgs[name] && cira_lib_darknet_widget::dlg_detect_lst_config->map_det_cfgs[name]->is_initialized) {
...
void DialogCTMVdoHeatmap::on_pushButton_config_clicked()
{
    bool pin_tmp = isPin;
    isPin = true;
...
void DialogCTMVdoHeatmap::current_config_changed(QString name) {
...
void DialogCTMVdoHeatmap::config_removed(QString name) {
...
void DialogCTMVdoHeatmap::cmdSetParameters(QJsonObject jso_cmd) {
...
void DialogCTMVdoHeatmap::on_pushButton_brows_vdo_clicked()
{
  bool tmp_pin = isPin;
  isPin = true;
  QString fileName = QFileDialog::getOpenFileName(
                  ...
                  tr("Video Files (*.dat *.wmv *.3gp  *.amv "
                                             ...
                                             "*.mp4v *.mpeg *.tod *.ts *.tts  *.webm")
                  , 0, QFileDialog::DontUseNativeDialog);

  isPin = tmp_pin;
  if(!fileName.isEmpty()) {
    tmp_video = fileName;
    ui->label_vdo_path->setText(tmp_video);
  }
}

void DialogCTMVdoHeatmap::on_doubleSpinBox_monitor_txt_scale_valueChanged(double arg1)
{
    (*mon_txt_scale) = arg1;
}
