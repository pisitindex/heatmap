...
FormCTMVdoHeatmap::FormCTMVdoHeatmap(QWidget *parent) :
  QWidget(parent),
  ui(new Ui::FormCTMVdoHeatmap)
{
  ui->setupUi(this);
  ...
  workerThread = new ThreadCTMVdoHeatmap();
  ...
  dialog = new DialogCTMVdoHeatmap();
...
FormCTMVdoHeatmap::~FormCTMVdoHeatmap()
{
...
void FormCTMVdoHeatmap::init() {
  ...
  if(!config_path.isEmpty() && workerThread->cira_yolo_->detector == NULL) {
    ...
    dlg_ld_cfg.setWindowTitle(" CiRA AI : CTMVdoHeatmap");
...
void FormCTMVdoHeatmap::on_pushButton_nodeEnable_clicked()
{
  if(nodeStatus_enable) {
    ui->pushButton_nodeEnable->setStyleSheet(style_nodeDisable);
...
void FormCTMVdoHeatmap::on_pushButton_prop_clicked()
{
