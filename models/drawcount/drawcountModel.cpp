...
void drawcountModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void drawcountModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void drawcountModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &drawcountModel::runProcess, _flowDataIn);
