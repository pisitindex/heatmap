#include "Realtime_HeatmapModel.hpp"
...
void Realtime_HeatmapModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void Realtime_HeatmapModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void Realtime_HeatmapModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &Realtime_HeatmapModel::runProcess, _flowDataIn);
