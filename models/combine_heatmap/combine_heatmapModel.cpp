...
void combine_heatmapModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
...
void combine_heatmapModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void combine_heatmapModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &combine_heatmapModel::runProcess, _flowDataIn);
