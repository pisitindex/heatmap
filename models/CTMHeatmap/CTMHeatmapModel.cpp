...
void CTMHeatmapModel::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
  ...
  if(form->workerThread->isUseImage) {
    flowdata._img = form->workerThread->mat_result;
  }
...
void CTMHeatmapModel::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
dataType(PortType portType, PortIndex portIndex) const
{
  switch (portType)
  {
    ...
    case PortType::Out:
      switch (portIndex)
      {
        case 0: return FlowData().type();
        case 1: return FlowData().type();
      }
      break;
...
void CTMHeatmapModel::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &CTMHeatmapModel::runProcess, _flowDataIn);
