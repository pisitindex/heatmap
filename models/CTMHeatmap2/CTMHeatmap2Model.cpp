...
void CTMHeatmap2Model::monitoring() {
...
void CTMHeatmap2Model::runProcess(std::shared_ptr<FlowData> _flowDataIn) {
  ...
  form->workerThread->mtx.lock();
  form->workerThread->run();
  form->workerThread->mtx.unlock();
...
void CTMHeatmap2Model::loopTimerLoadFromButton() {
  if(!GlobalData::isLoadingFlow) {
...
void CTMHeatmap2Model::setInData(std::shared_ptr<QtNodes::NodeData> data, PortIndex portIndex) {
  ...
  if(GlobalData::isAllSceneCanRun && form->nodeStatus_enable
    ...
    form->update_ui();
    QtConcurrent::run(this, &CTMHeatmap2Model::runProcess, _flowDataIn);
    //mutex.unlock();
